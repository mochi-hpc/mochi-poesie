...
TEST_CASE("Javascript vm test", "[javascript]") {
    ...
    const auto provider_config = R"(
    {
        "vm": {
            "type": "javascript",
            "config": {
                "preamble_file": "example-preamble.js"
            }
        }
    }
    )";
    ...
    SECTION("Create VmHandle") {
        ...
        SECTION("Execute code") {
            ...
            REQUIRE_NOTHROW([&]() { future = rh.execute("my_add(42,33)"); }());
        ...
        SECTION("Execute code with argv") {
            ...
            poesie::VmHandle::FutureType future;
            REQUIRE_NOTHROW([&]() { future = rh.execute("process.argv[1] + 33", argv); }());
        ...
        SECTION("Execute foreign function") {
            ...
            poesie::VmHandle::FutureType future;
            REQUIRE_NOTHROW([&]() { future = rh.execute("my_mult(42,33)"); }());
        ...
        SECTION("Load a file") {
            ...
            poesie::VmHandle::FutureType future;
            REQUIRE_NOTHROW([&]() { future = rh.load("example.js"); }());
        ...
        SECTION("Load a file (bad file)") {
            ...
            poesie::VmHandle::FutureType future;
            REQUIRE_NOTHROW([&]() { future = rh.load("bad.rb"); }());
        ...
        SECTION("Return JSON data") {
            ...
            auto expected = std::string{
                R"({"a":[2.0,4.0,6.0,8.0],"b":{"c":"abcd","d":"efgh"},)"
                R"("e":null,"x":42.0,"y":4.2,"z":true})"};
        ...
        SECTION("Call method on object") {
            ...
            REQUIRE_NOTHROW([&]() { rh.load("example-my-class.js").wait(); }());
            ...
            auto content = result.dump();
            auto expected = std::string{
                R"({"t":false,"transform":{},"x":13.0,"y":"hello world","z":[1.0,2.0,3.0]})"};
            REQUIRE(content == expected);
        }
