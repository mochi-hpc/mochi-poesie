...
TEST_CASE("Ruby vm test", "[ruby]") {
    ...
    const auto provider_config = R"(
    {
        "vm": {
            "type": "ruby",
            "config": {
                "preamble_file": "example-preamble.rb"
            }
        }
    }
    )";
    ...
    SECTION("Create VmHandle") {
        ...
        SECTION("Execute code with argv") {
            ...
            poesie::VmHandle::FutureType future;
            REQUIRE_NOTHROW([&]() { future = rh.execute("ARGV[1] + 33", argv); }());
        ...
        SECTION("Load a file") {
            ...
            poesie::VmHandle::FutureType future;
            REQUIRE_NOTHROW([&]() { future = rh.load("example.rb"); }());
        ...
        SECTION("Return JSON data") {
            ...
            auto expected = std::string{
                R"({"a":[2,4,6,8],"b":{"c":"abcd","d":"efgh"},)"
                R"("e":null,"x":42,"y":4.199999999999999,"z":true})"};
        ...
        SECTION("Call method on object") {
            ...
            REQUIRE_NOTHROW([&]() { rh.load("example-my-class.rb").wait(); }());
        ...
        SECTION("Use MemoryView") {
            ...
            auto code = R"(
              def use_memory_view(view)
                  raise "invalid view size" unless view.size == 16
                  str = view.to_s
